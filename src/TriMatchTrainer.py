...
def evaluate(dataStream, valid_graph, sess, outpath=None, label_vocab=None, mode='prediction',
             char_vocab=None, POS_vocab=None, NER_vocab=None, use_options=False):
...
#                          train_graph.get_in_passage_chars(): char_matrix_idx_2_batch, 
            ...
            if FLAGS.verbose:
                return_list = sess.run([train_graph.get_train_op(), train_graph.get_loss(), train_graph.get_predictions(),train_graph.get_prob(),
                    train_graph.final_logits, train_graph.correct]+train_graph.matching_vectors, feed_dict=feed_dict)
                print(len(return_list))
                _, loss_value, pred, prob, logits, correct=return_list[0:6] 
                print('pred=',pred)
                print('prob=',prob)
                print('logits=',logits)
    ...
    with tf.Graph().as_default():
        ...
        accuracy = evaluate(testDataStream, valid_graph, sess,char_vocab=char_vocab,POS_vocab=POS_vocab, NER_vocab=NER_vocab, use_options=FLAGS.use_options)
