...
if sentOnly:
	...
	output_file=open(r'D:\users\t-yicxu\data\squad\entail_'+mode+'_%s_%d_%s_%dclass_re%d.tsv' %(str(n_ans),n_sent,span_mode,num_class,repeat_gt),'w',encoding='utf-8')
...
for (ii,data) in enumerate(all_data['data']):
	...
	if target_num==0:
		continue
	first_ten_perm=np.random.permutation(n_best_choice)
	for tmpi in range(len(spans)):
		if tmpi<n_best_choice:
